...
// ── Geometry SoA tag constants (mirror geometry::flat) — used by the
//    walk helpers copied in for the fused CE transport kernel. ────────
...
const GMAX_DEPTH: u32 = 4;
#[allow(dead_code)]
const GMAX_DEPTH_USIZE: usize = 4;
const GSURF_STRIDE: u32 = 8;
...
// ── Geometry walk (copied from gpu_transport_cubecl; reads a geometry
//    SoA blob via offsets. Same validated source as the const-XS path.)
...
fn surf_eval(
    ...
    let p = (surf_params_off + s_idx * GSURF_STRIDE) as usize;
...
/// `st_*` arrays (length ≥ GMAX_DEPTH) and returns the depth (0 = leak).
...
/// / fission packers and records base offsets.
pub fn pack_ce_full(
    tables: &HostTables,
    geom: &Geometry,
    nuclides: &[NuclideCe],
...
fn ce_transport_kernel(
    ...
    if tid < n_particles as usize {
        ...
        // geometry stack.
        let mut st_cell = Array::<i32>::new(GMAX_DEPTH_USIZE);
        let mut st_offx = Array::<f64>::new(GMAX_DEPTH_USIZE);
        let mut st_offy = Array::<f64>::new(GMAX_DEPTH_USIZE);
        let mut st_offz = Array::<f64>::new(GMAX_DEPTH_USIZE);
        let mut st_has_lat = Array::<i32>::new(GMAX_DEPTH_USIZE);
        let mut st_lat_id = Array::<i32>::new(GMAX_DEPTH_USIZE);
        let mut st_lat_ix = Array::<i32>::new(GMAX_DEPTH_USIZE);
        let mut st_lat_iy = Array::<i32>::new(GMAX_DEPTH_USIZE);
        let mut st_lat_iz = Array::<i32>::new(GMAX_DEPTH_USIZE);
            ...
            if alive == 1u32 && ev < max_events {
                ...
                if mat < 0i32 {
                    ...
                    if dist >= f64::new(1e29) {
                        ...
                        ce_cross(idata, fdata, surf_params_off, surf_type_off, bc, surf_idx, dist,
...
/// Boundary crossing for the CE kernel (vacuum kill / reflect / transmit).
#[cube]
#[allow(clippy::too_many_arguments)]
fn ce_cross(
    idata: &Array<i32>,
