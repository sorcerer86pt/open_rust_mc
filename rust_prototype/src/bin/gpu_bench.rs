...
mod cuda_main {
    ...
    fn run_single_nuclide_benchmark(args: &Args) {
        ...
        // Warmup
        let _ = gpu.reconstruct_batch(basis, coeffs, &energy_indices[..1000.min(args.particles)], n_e, rank);
        ...
        // Comparison
        print_comparison("U-235 fission", cpu_ns, gpu_ns, &cpu_results, &gpu_results, args.particles);
    }
    ...
    fn run_pwr_benchmark(args: &Args) {
        ...
        for (filename, kernels) in &all_kernels {
            ...
            for (rxn_name, kernel_opt) in reactions {
                ...
                // Max relative error
                let max_err = cpu_results.iter().zip(gpu_results.iter())
                    .map(|(&c, &g)| (c - g).abs() / c.max(1e-30))
                    .fold(0.0_f64, f64::max);

                let speedup = cpu_ns / gpu_ns;
                println!("{:<12} {:<10} {:>10.1} {:>10.1} {:>7.1}x {:>10.2e}",
        ...
        println!("{:<12} {:<10} {:>10.1} {:>10.1} {:>7.1}x",
                 "AVERAGE", &format!("({total_reactions} rxn)"), avg_cpu, avg_gpu, avg_speedup);

        println!("\nPWR collision cost estimate ({n_particles} particles):");
        println!("  CPU: {:.1} ns/particle (sum of all nuclides/reactions)",
                 total_cpu_ns);
        println!("  GPU: {:.1} ns/particle (sum of all nuclides/reactions)",
                 total_gpu_ns);
    ...
    fn print_comparison(label: &str, cpu_ns: f64, gpu_ns: f64,
                       cpu_results: &[f64], gpu_results: &[f64], n: usize) {
        let speedup = cpu_ns / gpu_ns;
        println!("\n{}", "=".repeat(50));
        println!("  {label}:");
        ...
        println!("  Max relative error (first {}): {:.2e}", n.min(10), max_err);
    }
}
