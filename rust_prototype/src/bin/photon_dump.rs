...
fn main() -> ExitCode {
    ...
    println!(
        "Element {:>2} (Z={}): {} energy points, {:.3e} to {:.3e} eV",
    ...
    for s in &elem.subshells {
        println!(
            "  {:<4}  binding {:>10.2} eV   n_e = {:>5.2}   xs_pts = {:>4}   transitions = {:>3}",
            s.designator,
            s.binding_energy,
            s.num_electrons,
            s.xs.len(),
    ...
    println!();
    println!(
        "Coherent form factor F(x,Z):                {} points, x in [{:.3e}, {:.3e}] 1/Å",
    ...
    println!(
        "Coherent integrated F²(x,Z) cumulative:     {} points",
        elem.coherent_integrated_form_factor.x.len()
    );
    println!(
        "Coherent anomalous f'(E):                   {} points, E in [{:.3e}, {:.3e}] eV",
        elem.coherent_anomalous.real.grid.len(),
        elem.coherent_anomalous
            .real
            .grid
            .first()
            .copied()
    ...
    println!(
        "Incoherent scattering factor S(x,Z):        {} points, x in [{:.3e}, {:.3e}] 1/Å",
        elem.incoherent_scattering_factor.x.len(),
        elem.incoherent_scattering_factor
            .x
            .first()
            .copied()
            .unwrap_or(0.0),
    ...
    println!(
        "Compton profiles: {} shells, {} pz points, total occupancy = {:.2} (Z={})",
        cp.n_shells(),
        cp.n_pz(),
        total_occ,
        elem.z
    ...
    println!(
        "Bremsstrahlung DCS grid: {} electron-E × {} photon-E (I = {:.1} eV)",
        br.electron_energy.len(),
        br.photon_energy.len(),
        br.mean_excitation_energy
    );
    println!(
        "  electron T_e in [{:.3e}, {:.3e}] eV",
        br.electron_energy.first().copied().unwrap_or(0.0),
        br.electron_energy.last().copied().unwrap_or(0.0)
    );
    println!(
        "  photon k/T_e in [{:.3e}, {:.3e}] (scaled)",
