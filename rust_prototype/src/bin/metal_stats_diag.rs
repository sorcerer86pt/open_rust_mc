...
impl Active {
    ...
    fn report(&self, label: &str, particles_per_batch: u64) {
        ...
        println!(
            "--- {label} active-batch means (over {} batches, {:.0} source histories) ---",
            self.n, n_source
        );
        println!(
            "  k_eff             : {mean:.5} ± {stderr:.5}   ({:+.0} pcm vs 1.0)",
            (mean - 1.0) * 1e5
        );
        println!(
            "  leakage / source  : {:.4}    ({} / {:.0})",
            self.leak_sum as f64 / n_source,
            self.leak_sum,
            n_source
        );
        ...
        println!(
            "  captures / source : {:.4}    ({})",
            self.abs_sum as f64 / n_source,
            self.abs_sum
        );
        println!(
            "  abs (OpenMC-def)  : {:.4}    ({} = captures + fissions)",
            abs_omc_style as f64 / n_source,
            abs_omc_style
        );
        println!(
            "  fissions / source : {:.4}    ({})",
            self.fis_sum as f64 / n_source,
            self.fis_sum
        );
        println!(
            "  collisions / src  : {:.2}    ({})",
            self.coll_sum as f64 / n_source,
            self.coll_sum
        );
        println!(
            "  surf cross / src  : {:.2}    ({})",
            self.surf_sum as f64 / n_source,
            self.surf_sum
        );
        println!(
            "  thermal scat / src: {:.4}   ({})",
            self.therm_sum as f64 / n_source,
            self.therm_sum
        );
        ...
        if self.el_sum + self.inel_sum + self.cap_sum > 0 {
            ...
            println!(
                "    elastic   / src : {:.4}    ({} events)",
                el / n_source,
                self.el_sum
            );
            println!(
                "    inelastic / src : {:.4}    ({} events)",
                inel / n_source,
                self.inel_sum
            );
            if self.inel_cont_sum > 0 {
                ...
                println!(
                    ...
                    disc / n_source,
                    self.inel_sum - self.inel_cont_sum,
                    100.0 * disc / inel,
                );
            }
            println!(
                "    fission   / src : {:.4}    ({} events)",
                fis / n_source,
                self.fis_sum
            );
            println!(
                "    capture   / src : {:.4}    ({} events)",
                cap / n_source,
                self.cap_sum
            );
            ...
            println!(
                "    (n2n+n3n+...) / src = collisions − (el+inel+fis+cap) = {:.4}   reconciliation {:.4} of total coll",
                (self.coll_sum as f64 - sum) / n_source,
                recon
            );
            ...
            println!(
                "    extra-n bank / src : {:.6}   (1·n2n + 2·n3n — GPU routes to fission source, CPU in-generation)",
                banked / n_source
            );
            if self.nxn_out_sum > 0 {
                ...
                println!(
                    "    nxn ⟨E_out⟩    : {:.4e} eV   σ = {:.4e}   ({} outgoing n)",
                    m, s, self.nxn_out_sum
                );
            }
...
fn report_delta(cpu: &Active, gpu: &Active, particles_per_batch: u64) {
    ...
    // refill factor.
    let nps = if cpu.hist_sum > 0 {
        cpu.hist_sum as f64
    } else {
        (cpu.n as f64) * particles_per_batch as f64
    };
    let nps_g = if gpu.hist_sum > 0 {
        gpu.hist_sum as f64
    } else {
        (gpu.n as f64) * particles_per_batch as f64
    };
    println!("\n=== Δ (GPU − CPU) ===");
    ...
    println!(
        "  Δ leakage/src: {:+.4}  ({:+.2}%)   cpu={:.4}  gpu={:.4}",
        gpu_leak - cpu_leak,
        diff_pct(cpu_leak, gpu_leak),
        cpu_leak,
        gpu_leak
    );
    ...
    println!(
        "  Δ abs/src    : {:+.4}  ({:+.2}%)   cpu={:.4}  gpu={:.4}",
        gpu_abs - cpu_abs,
        diff_pct(cpu_abs, gpu_abs),
        cpu_abs,
        gpu_abs
    );
    ...
    println!(
        "  Δ fis/src    : {:+.4}  ({:+.2}%)   cpu={:.4}  gpu={:.4}",
        gpu_fis - cpu_fis,
        diff_pct(cpu_fis, gpu_fis),
        cpu_fis,
        gpu_fis
    );
    ...
    println!(
        "  Δ coll/src   : {:+.2}   ({:+.2}%)   cpu={:.2}  gpu={:.2}",
        gpu_col - cpu_col,
        diff_pct(cpu_col, gpu_col),
        cpu_col,
        gpu_col
    );
    ...
    println!(
        "  Δ surf/src   : {:+.2}   ({:+.2}%)   cpu={:.2}  gpu={:.2}",
        gpu_surf - cpu_surf,
        diff_pct(cpu_surf, gpu_surf),
        cpu_surf,
        gpu_surf
    );
    ...
    println!(
        "  Δ therm/src  : {:+.4} ({:+.2}%)   cpu={:.4}  gpu={:.4}",
        gpu_therm - cpu_therm,
        diff_pct(cpu_therm, gpu_therm),
        cpu_therm,
        gpu_therm
    );
    ...
    println!(
        "  Δ n2n/src    : {:+.6} ({:+.2}%)   cpu={:.6}  gpu={:.6}",
        gpu_n2n - cpu_n2n,
        diff_pct(cpu_n2n, gpu_n2n),
        cpu_n2n,
        gpu_n2n
    );
    ...
    println!(
        "  Δ n3n/src    : {:+.6} ({:+.2}%)   cpu={:.6}  gpu={:.6}",
        gpu_n3n - cpu_n3n,
        diff_pct(cpu_n3n, gpu_n3n),
        cpu_n3n,
        gpu_n3n
    );
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "settings: batches={arg_b} inactive={arg_i} particles={arg_p} seed={arg_s} rank={arg_r} refill={arg_refill:?} auto_refill={arg_auto_refill} nxn_mode={arg_nxn_mode} maxev={arg_maxev}"
    );
    ...
    let resolved = material_resolve::resolve_materials(&loaded.materials, &lib, arg_r)?;
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        let runner = CudaRunner {
            ...
            initial_source: Box::new(move |n, s| {
                let sites =
                    open_rust_mc::transport::simulate::initial_source(n, &geometry, &cells, s);
                sites
                    .iter()
        ...
        let openmc_path = match &arg_openmc {
            ...
            None => workspace_root()
                .join("outputs")
                .join("openmc_godiva_tallies.json"),
        };
        if let Ok(text) = std::fs::read_to_string(&openmc_path) {
            ...
            println!(
                "  k_eff             : {k_omc:.5} ± {sigma_seeds:.5} (seed σ)   ({:+.0} pcm vs 1.0)",
                (k_omc - 1.0) * 1e5
            );

            // Leakage current per source — OpenMC tally "leakage", current score.
            if let Some(leak_t) = v["tallies_seed_mean"].get("leakage") {
                if let Some(mean) = leak_t["mean"]
                    .as_array()
                    .and_then(|a| a.first())
                    .and_then(|x| x.as_f64())
                {
                    println!(
                        "  leakage / source  : {mean:.4}    (OpenMC current score, per source particle)"
                    );
                }
            }
            ...
            if let Some(nuf) = v["tallies_seed_mean"].get("rate_nu-fission") {
                if let Some(arr) = nuf["mean"].as_array() {
                    ...
                    println!(
                        "  ν·fissions / src  : {total:.4}    (OpenMC nu-fission summed across nuclides)"
                    );
                }
            }
            ...
            if let Some(t) = v["tallies_seed_mean"].get("rate_by_energy") {
                ...
                {
                    let groups: Vec<f64> = groups_arr.iter().filter_map(|g| g.as_f64()).collect();
                    ...
                    let fine = (|| {
                        ...
                        let edges: Vec<f64> = edges.iter().filter_map(|x| x.as_f64()).collect();
                        let rates: Vec<f64> = rates_arr.iter().filter_map(|x| x.as_f64()).collect();
                    ...
                    println!(
                        ...
                        m_fis,
                        s_fis,
                        if m_fis > 0.0 { s_fis / m_fis } else { 0.0 }
                    );
                    println!("    scatter : ⟨E⟩ = {:.4e}   σ = {:.4e}", m_sc, s_sc);
                    println!("    total   : ⟨E⟩ = {:.4e}   σ = {:.4e}", m_tot, s_tot);
                    ...
                    if gpu_s > 0.0 || cpu_s > 0.0 {
                        ...
                        println!(
                            ...
                            omc_m,
                            omc_s,
                            if omc_m > 0.0 { omc_s / omc_m } else { 0.0 }
                        );
                        if gpu_s > 0.0 {
                            println!(
                        ...
                        if cpu_s > 0.0 && gpu_s > 0.0 {
                            println!(
                                ...
                                gpu_s - cpu_s,
                                if cpu_s > 0.0 {
                                    (gpu_s - cpu_s) / cpu_s * 100.0
                                } else {
                                    0.0
                                }
                            );
                        }
                        println!("    → If GPU σ ≈ CPU σ but both differ from OpenMC, the bias");
                        println!("      is a Rust-engine effect shared by both backends. If GPU");
                        println!("      σ differs from CPU σ, the bias is GPU-only and lives");
                        println!("      in event ordering / float-rounding / kernel layout.");
                    }
                }
            }
            ...
            println!(
                "  k_eff Δ        : CPU {:+.0} pcm   GPU {:+.0} pcm   (GPU − CPU = {:+.0} pcm)",
                (cpu_k - k_omc) * 1e5,
                (gpu_k - k_omc) * 1e5,
                (gpu_k - cpu_k) * 1e5
            );
            if let Some(leak_t) = v["tallies_seed_mean"].get("leakage") {
                if let Some(leak_omc) = leak_t["mean"]
                    .as_array()
                    .and_then(|a| a.first())
                    .and_then(|x| x.as_f64())
                {
                    let nps = if cpu_act.hist_sum > 0 {
                        cpu_act.hist_sum as f64
                    } else {
                        (cpu_act.n as f64) * ppb as f64
                    };
                    let nps_g = if gpu_act.hist_sum > 0 {
                        gpu_act.hist_sum as f64
                    } else {
                        (gpu_act.n as f64) * ppb as f64
                    };
                    let cpu_leak = cpu_act.leak_sum as f64 / nps;
                    let gpu_leak = gpu_act.leak_sum as f64 / nps_g;
                    println!(
                        "  leakage/src Δ  : CPU {:+.4}   GPU {:+.4}   (cpu={:.4} gpu={:.4} omc={:.4})",
                        cpu_leak - leak_omc,
                        gpu_leak - leak_omc,
                        cpu_leak,
                        gpu_leak,
            ...
            // mismatch can't be misread as a physics difference.
            {
                    ...
                    (sum_tally("rate_absorption"), sum_tally("rate_fission"))
                {
                    ...
                    println!(
                        "  ─ absorption-bucket reconciliation (read THIS before quoting a capture gap) ─"
                    );
                    println!(
                        "    OpenMC: total_abs {omc_abs:.4} = fission {omc_fis:.4} + (n,γ) {omc_ngamma:.4} + charged {omc_charged:.4}"
                    );
                    println!(
                        "    OpenMC capture-equiv ((n,γ)+charged = total_abs − fis) : {omc_cap_equiv:.4}"
                    );
                    println!(
                        "    our \"capture\" bucket (ALL non-fission abs)            : CPU {cpu_cap:.4}   GPU {gpu_cap:.4}"
                    );
                    println!(
                        "      → Δ capture-equiv : CPU {:+.4}   GPU {:+.4}   (compare vs capture-equiv, NOT bare (n,γ))",
                        cpu_cap - omc_cap_equiv,
                        gpu_cap - omc_cap_equiv
                    );
                    println!(
                        "    total absorption (cap + fis)                          : CPU {cpu_totabs:.4}   GPU {gpu_totabs:.4}   omc {omc_abs:.4}"
                    );
                    println!(
                        "      → Δ total-abs     : CPU {:+.4}   GPU {:+.4}",
                        cpu_totabs - omc_abs,
                        gpu_totabs - omc_abs
                    );
                }
            ...
            println!(
                "\n(OpenMC reference JSON not found at {})",
                openmc_path.display()
            );
