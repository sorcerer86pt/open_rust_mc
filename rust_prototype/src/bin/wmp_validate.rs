...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "#   E_min = {:.6e} eV, E_max = {:.6e} eV",
        wmp.e_min, wmp.e_max
    );
    println!(
        "#   n_poles = {}, n_windows = {}, fit_order = {}",
        wmp.n_poles, wmp.n_windows, wmp.fit_order
    );
    println!(
        "#   sqrtAWR = {:.6e}, fissionable = {}",
    ...
    let energies: Vec<f64> = vec![
        0.025, // thermal
        1.0, 6.674, // first U-238 resonance
        10.0, 20.9, 36.7, 50.0, 100.0, 500.0, 1_000.0, 5_000.0, 10_000.0, 19_000.0,
    ];
    ...
    for &e in &energies {
        ...
        println!("{:14.6e} {:16.8e} {:16.8e} {:16.8e}", e, ss, sa, sf);
    }
