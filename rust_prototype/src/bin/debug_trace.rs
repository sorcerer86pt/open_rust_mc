...
fn main() {
    ...
    let provider = xs_provider::SvdXsProvider { nuclides: kernels, thermal: vec![] };

    // === CPU XS values at test energies ===
    println!("\n── CPU XS at test energies ──");
    let mut f = std::fs::File::create("diag_cpu_xs.csv").unwrap();
    writeln!(f, "nuc,energy,elastic,inelastic,n2n,n3n,fission,capture,total").unwrap();
    let test_energies = [1e-2, 1e0, 1e2, 1e3, 5e3, 1e4, 1e5, 5e5, 1e6, 2e6, 5e6, 1e7];
    for nuc_idx in 0..3 {
        for &e in &test_energies {
            let xs = provider.lookup(nuc_idx, e);
            writeln!(f, "{},{:.2e},{:.6e},{:.6e},{:.6e},{:.6e},{:.6e},{:.6e},{:.6e}",
                NUCLIDES[nuc_idx].0, e, xs.elastic, xs.inelastic, xs.n2n, xs.n3n,
                xs.fission, xs.capture, xs.total).unwrap();
        }
    }
...
fn sample_mu_manual(ang: &AngularDistribution, energy: f64, xi: f64) -> f64 {
    if ang.energies.is_empty() { return 2.0 * xi - 1.0; }
    ...
    if energy >= ang.energies[n - 1] {
        return sample_cdf(&ang.distributions[n-1].mu, &ang.distributions[n-1].cdf, xi);
    }
    let mut lo = 0usize;
    let mut hi = n - 1;
    while hi - lo > 1 {
        let mid = (lo + hi) / 2;
        if ang.energies[mid] <= energy { lo = mid; } else { hi = mid; }
    }
    let idx = lo;
    if idx + 1 >= n {
    ...
    let mu_hi = sample_cdf(&ang.distributions[idx+1].mu, &ang.distributions[idx+1].cdf, xi);
    ((1.0 - frac) * mu_lo + frac * mu_hi).clamp(-1.0, 1.0)
}

fn sample_cdf(mu: &[f64], cdf: &[f64], xi: f64) -> f64 {
    let n = cdf.len();
    if n < 2 { return 2.0 * xi - 1.0; }
    let mut lo = 0usize;
    let mut hi = n - 1;
    while hi - lo > 1 {
        let mid = (lo + hi) / 2;
        if cdf[mid] <= xi { lo = mid; } else { hi = mid; }
    }
...
fn run_gpu_comparison(_args: &Args, provider: &xs_provider::SvdXsProvider) {
    ...
    let gpu = match GpuTransportContext::new() {
        Ok(g) => g,
        Err(e) => { eprintln!("  GPU init failed: {e}"); return; }
    };
    let nuc_data = gpu.upload_nuclide_data(&provider.nuclides, 5).expect("upload nuclide");
    ...
    let mat_data = gpu.upload_material_data(&[heu], &awrs, &nu_bars).expect("upload mat");
    let sab_data = gpu.upload_sab_data_empty().expect("empty sab");

    let mut f = std::fs::File::create("diag_gpu_vs_cpu_angular.csv").unwrap();
    writeln!(f, "nuc,energy,xi,cpu_mu,gpu_stairstep,gpu_interp,diff_ss,diff_interp").unwrap();
    ...
    for nuc_idx in 0..3usize {
        ...
        let (gpu_ss, gpu_interp) = gpu.debug_angular_sample(
            &test_energies, &test_xis, nuc_idx as i32,
            &nuc_data, &mat_data, &sab_data, 1,
        ...
        for i in 0..test_energies.len() {
            ...
            total += 1;
            writeln!(f, "{},{:.6e},{:.6},{:.10},{:.10},{:.10},{:.2e},{:.2e}",
                NUCLIDES[nuc_idx].0, test_energies[i], test_xis[i], cpu_mu,
                gpu_ss[i], gpu_interp[i], (gpu_ss[i]-cpu_mu).abs(), diff_interp).unwrap();
        }
    }
    ...
    for nuc_idx in 0..3usize {
        let gpu_xs = gpu.debug_xs_reconstruct(
            &test_energies, nuc_idx as i32, &nuc_data, &mat_data, &sab_data, 1,
        ).expect("GPU XS");
        for (ei, &e) in test_energies.iter().enumerate() {
            let cpu = provider.lookup(nuc_idx, e);
            let cpu_vals = [cpu.elastic, cpu.inelastic, cpu.n2n, cpu.n3n, cpu.fission, cpu.capture];
            for r in 0..6 {
                let cv = cpu_vals[r]; let gv = gpu_xs[ei*6+r];
                let rel = if cv.abs() > 1e-30 { ((gv-cv)/cv).abs() }
                    else if gv.abs() > 1e-30 { 1.0 } else { 0.0 };
                if rel > 1e-6 { n_mismatch += 1; }
                max_rel = max_rel.max(rel);
                writeln!(f, "{},{:.2e},{},{:.6e},{:.6e},{:.2e}",
                    NUCLIDES[nuc_idx].0, e, rxn_names[r], cv, gv, rel).unwrap();
            }
        }
    }
    ...
    if max_rel < 1e-4 {
        ...
        println!("  XS reconstruction: FAIL (max rel diff {:.2e}, {} mismatches)", max_rel, n_mismatch);
    ...
    let grid_offsets: Vec<i32> = gpu.stream().clone_dtoh(&nuc_data.grid_offsets).expect("dtoh");
    let n_energies: Vec<i32> = gpu.stream().clone_dtoh(&nuc_data.n_energies).expect("dtoh");
    let grids: Vec<f64> = gpu.stream().clone_dtoh(&nuc_data.all_energy_grids).expect("dtoh");
    for nuc_idx in 0..3usize {
        ...
        for &e in &[1e3_f64, 5e3, 1e4, 1e5, 1e6] {
            let gpu_idx = {
                if e <= gpu_grid[0] { 0 }
                else if e >= gpu_grid[n_e-1] { n_e-1 }
                else {
                    let (mut lo, mut hi) = (0, n_e-1);
                    while hi-lo > 1 { let mid=(lo+hi)/2; if gpu_grid[mid]<=e {lo=mid} else {hi=mid} }
                    lo
                }
            };
            let cpu_idx = provider.nuclides[nuc_idx].elastic.as_ref()
            ...
            if gpu_idx != cpu_idx {
                println!("  {} E={:.0e}: gpu_idx={} cpu_idx={} MISMATCH", NUCLIDES[nuc_idx].0, e, gpu_idx, cpu_idx);
                all_ok = false;
            }
        }
        if all_ok { println!("  {}: all energy indices match", NUCLIDES[nuc_idx].0); }
    }
}
