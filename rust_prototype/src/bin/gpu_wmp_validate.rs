...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("usage: gpu_wmp_validate <path/to/wmp/ZZAAA.h5> \
    ...
    println!("  E_min={:.3e}, E_max={:.3e}, n_poles={}, n_windows={}, fit_order={}",
             wmp.e_min, wmp.e_max, wmp.n_poles, wmp.n_windows, wmp.fit_order);

    // Test energy list
    let energies: Vec<f64> = vec![
        0.025, 1.0, 6.674, 10.0, 20.9, 36.7, 50.0, 100.0, 500.0,
        1_000.0, 5_000.0, 10_000.0,
    ].into_iter().filter(|&e| e >= wmp.e_min && e <= wmp.e_max).collect();
    let t_kelvin = 293.6;
    let n = energies.len();
    ...
    let poles_f64: Vec<f64> = wmp.poles.iter()
    ...
    let bench_energies: Vec<f64> = (0..n_bench).map(|i| {
        let f = if n_bench > 1 { i as f64 / (n_bench - 1) as f64 } else { 0.0 };
    ...
    let cfg_b = LaunchConfig { grid_dim: grid_b, block_dim: block_b, shared_mem_bytes: 0 };

    let n_bench_i32 = n_bench as i32;
    // Warm-up launch
    unsafe {
        stream.launch_builder(&kernel)
            .arg(&n_bench_i32).arg(&d_be).arg(&t_kelvin)
            .arg(&wmp.e_min).arg(&wmp.e_max).arg(&wmp.spacing).arg(&wmp.sqrt_awr)
            .arg(&nw).arg(&fo).arg(&fiss)
            .arg(&d_poles_f64).arg(&d_windows).arg(&d_broaden).arg(&d_curvefit)
            .arg(&mut d_bs).arg(&mut d_ba).arg(&mut d_bf).launch(cfg_b)?;
    }
    stream.synchronize()?;
    ...
    println!("  CPU single-thread: {:.1} ns/lookup ({} points, {:.1} ms)",
             cpu_ns_per, cpu_energies.len(), cpu_ms);
    println!("  GPU (this device): {:.1} ns/lookup ({} points × {} launches, {:.1} ms total)",
             gpu_ns_per, n_bench, n_rep, gpu_ms);
    println!("  GPU / CPU speedup: {:.1}×", cpu_ns_per / gpu_ns_per);

    // CPU reference
    println!();
    println!("{:>14} {:>14} {:>14} {:>10} {:>14} {:>14} {:>10}",
             "E (eV)", "cpu σ_abs", "gpu σ_abs", "Δ rel",
             "cpu σ_fis", "gpu σ_fis", "Δ rel");
    ...
    for (i, &e) in energies.iter().enumerate() {
        ...
        println!("{:14.3e} {:14.6e} {:14.6e} {:10.2e} {:14.6e} {:14.6e} {:10.2e}",
