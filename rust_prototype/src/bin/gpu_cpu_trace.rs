...
mod cuda_main {
    ...
    fn load_thermal(data_dir: &PathBuf) -> Vec<Option<Arc<ThermalScatteringData>>> {
        ...
        let h2o_thermal: Option<Arc<ThermalScatteringData>> = if h2o_path.exists() {
            hdf5_reader::load_thermal_scattering(&h2o_path)
                .ok()
                .map(Arc::new)
        } else {
            None
    ...
    fn cpu_trace_particle(
        ...
        let cell_idx =
            geometry::ray::find_cell(Vec3::new(source.0, source.1, source.2), surfaces, cells)
                .unwrap_or(0);
        ...
        for _step in 0..max_steps {
            ...
            let mat_idx = match cell.fill {
                ...
                CellFill::Void => {
                    ...
                    row[1] = particle.pos.x;
                    row[2] = particle.pos.y;
                    row[3] = particle.pos.z;
            ...
            if use_thermal {
                ...
                if xi < cum {
                    ...
                    if particle.energy < 400.0 * cell_kt {
                        ...
                        particle.energy = (particle.energy * (1.0 + alpha + (1.0 - alpha) * mu_cm)
                            / 2.0)
                            .max(1e-11);
    ...
    pub fn run() {
        ...
        let nuc_data = gpu
            .upload_nuclide_data(&svd_kernels, args.rank)
            .expect("upload nuc");
        ...
        for pid in 0..n.min(20) {
            ...
            for s in 0..min_steps {
                ...
                let e_diff = if cpu_e > 0.0 {
                    ((gpu_e - cpu_e) / cpu_e).abs()
                } else {
                    0.0
                };
                let mt_diff = if cpu_mt > 0.0 {
                    ((gpu_mt - cpu_mt) / cpu_mt).abs()
                } else {
                    0.0
                };
            ...
            if let Some(s) = first_diff_step {
                let gb = pid * max_steps * cols + s * cols;
                println!(
                    "  P{pid:>3} diverges at step {s}: GPU E={:.4e} ev={} mt={:.4e} | CPU E={:.4e} ev={} mt={:.4e}",
                    gpu_trace.data[gb],
                    gpu_trace.data[gb + 9] as i32,
                    gpu_trace.data[gb + 6],
                    cpu_rows[s][0],
                    cpu_rows[s][9] as i32,
                    cpu_rows[s][6],
                ...
                if n_diverge == 0 {
                    ...
                    println!(
                        "    step |  E(GPU)      E(CPU)     | cell  mat | d_coll(GPU)  d_coll(CPU) | d_surf(GPU) d_surf(CPU) | ev(G) ev(C) | mt(GPU)    mt(CPU)    | E_out(GPU)  E_out(CPU)"
                    );
                    for st in start..end {
                        ...
                        println!(
                            "    {:>4} | {:11.4e} {:11.4e} | {:>4} {:>4} | {:11.5e} {:11.5e} | {:11.5e} {:11.5e} | {:>5} {:>5} | {:10.6e} {:10.6e} | {:11.4e} {:11.4e}{}",
                            st,
                            gpu_trace.data[g],
                            c[0], // energy
                            gpu_trace.data[g + 4] as i32,
                            c[4] as i32, // cell
                            gpu_trace.data[g + 7],
                            c[7], // d_coll
                            gpu_trace.data[g + 8],
                            c[8], // d_surf
                            gpu_trace.data[g + 9] as i32,
                            c[9] as i32, // event
                            gpu_trace.data[g + 6],
                            c[6], // macro_total
                            gpu_trace.data[g + 15],
        ...
        println!(
            "\n  Match: {n_match}, Diverge: {n_diverge} (of {} checked)",
            n.min(20)
        );
    }
}
