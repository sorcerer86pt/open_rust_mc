...
// Backend-agnostic geometry → SoA flattening. Lifted into
// `geometry::flat` so the CubeCL renderer shares one definition of the
// device layout; the tag constants there are the single source of
// truth this CUDA path and the CubeCL kernel both mirror.
