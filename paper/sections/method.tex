...
and is the single change that moves the off-library PWR
result from RED to GREEN on the verdict semaphore.

\subsection{Partition-of-unity Ducru for off-library $T$}
...
out at $900$~K we measured the non-negative simplex
projection~\cite{duchi2008simplex} of the raw Ducru
weights; the clip-and-renormalize is worse than the signed
scheme, not better, raising the L2 reconstruction error
from $1.5\%$ to $5\%$ (Sec.~\ref{sec:off_library_qp}). The
fully constrained kernel-QP remains future work; for the
systems we benchmarked, Eq.~\ref{eq:ducru_unity} suffices.
