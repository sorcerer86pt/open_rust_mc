...
industry baseline at matching statistics. A high-statistics
ten-seed redo at the matched $150\times 50\,000$
configuration is the outstanding follow-up.
...
Hybrid in-solver, full SVD grid (SVD basis + WMP payload) & $519.0$ & $5.2\times$ \emph{larger} \\
Hybrid in-solver, smooth-only rebuild (live) & $487.6$ & $4.8\times$ \emph{larger} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{verbatim}
Memory (before smooth-only rebuild):
  full SVD basis     = 517 870.5 KB
  WMP payload        =   1 153.8 KB
  TOTAL (full grid)  = 519 024.3 KB
Memory (after smooth-only rebuild — actually realised):
  smooth SVD basis   = 486 447.4 KB
  WMP payload        =   1 153.8 KB
  TOTAL (smooth)     = 487 601.2 KB
  rebuild dropped    =  31 423.1 KB across elastic/fission/capture
  reduction vs full  = 1.06×
\end{verbatim}
...
smooth-only rebuild we implemented is item (iii) restricted
to MT$=2$, MT$=18$, MT$=102$; extending it to the rest of
the reaction set is engineering work.
...
(b)~end-to-end $k_\infty$ at single-seed statistics
agrees with ACE+WMP within $-26$~pcm
($\ll 1\sigma_\text{seed}$), with the ten-seed redo at
matched $150\times 50\,000$ statistics as the outstanding
follow-up;
(c)~for a future full-core engine where the cascade
reactions are stored more densely than per-level SVD (e.g.
nuclear-temperature evaporation), the RRR saving becomes
proportionally more valuable. We report the hybrid as a
correctness deliverable and as scaffolding for those future
improvements, not as a memory or throughput win on the
present engine.
