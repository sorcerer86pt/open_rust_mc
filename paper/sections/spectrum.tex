...
Those same four channels are the ones that dominate
$k_\text{eff}$-relevant physics, so the rank-one fact about
the other $43$ is a structural observation about
ENDF/B-VII.1, not a shipping configuration: a production
engine must run $k \geq 3$ across \emph{all} reactions for
the four hard channels, which is the configuration used in
every benchmark in this paper.
...
eight reactions. Full energy grid, $T = 294$~K. Large-L3
system (Ryzen~$9800$X3D) single-core.}
\label{tab:xs_pareto}
\begin{tabular}{@{}cccc@{}}
\toprule
