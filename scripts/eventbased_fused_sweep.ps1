...
.SYNOPSIS
    ...
    after gr_scan_offsets absorbs the per-step d_type_count zeroing.
#>
...
$Csv = Join-Path $OutputsDir "eventbased_fused_a1000.csv"
$LogPath = Join-Path $OutputsDir "eventbased_fused_a1000.log"

"particles,k_inf,ns_per_particle,total_sim_ms,active_histories" | Out-File -Encoding ascii $Csv
"# eventbased fused-scan sweep - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File -Encoding ascii $LogPath
