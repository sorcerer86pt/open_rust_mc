...
# Variant table — keep the structure so adding -RefillFactor 4.0,
# 8.0 etc. is a one-line extension.
$variants = @(
    [PSCustomObject]@{ Tag = 'refill_off';                       Factor = 0.0          },
    [PSCustomObject]@{ Tag = ("refill_{0}x" -f $RefillFactor);   Factor = $RefillFactor }
)
...
foreach ($v in $variants) {
    ...
    Write-Host ""
    Write-Host ("=" * 70)
    Write-Host ("Running variant: {0}  (RefillFactor={1})" -f $v.Tag, $v.Factor)
    Write-Host ("=" * 70)
    & pwsh -ExecutionPolicy Bypass -File $inner `
        -Cases @($Case) `
        -Particles $Particles `
        -Batches $Batches `
        -Inactive $Inactive `
